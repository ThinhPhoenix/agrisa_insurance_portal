...
const PolicyDetailPage = ({ params }) => {
    ...
    const { cancelBasePolicyAPI, cancelLoading, cancelError } =
        useCancelPolicy();
        ...
        const loadPolicyDetail = async () => {
            ...
            try {
                ...
                if (policyStatus === "draft") {
                    ...
                    policyData = await fetchActivePolicyDetail(params.id);
                    if (!policyData) {
                        policyData = await fetchDraftPolicyDetail(
                            params.id,
                            false
                        );
                    }
                ...
                const hasAccess =
                    policyProviderId === partnerId ||
                    policyProviderId === userId;
    ...
    useEffect(() => {
        const loadDataSources = async () => {
            ...
            const dataSourceDetails = await fetchDataSourcesByIds(
                dataSourceIds
            );
        ...
        configuration: {
            ...
            triggerConditions: conditions.map((condition) => {
                const dataSourceDetail =
                    enrichedDataSources[condition.data_source_id];
                return {
                    id: condition.id,
                    ...
                    dataSourceLabel:
                        dataSourceDetail?.label || condition.data_source_id,
                    dataSourceParameterName:
                        dataSourceDetail?.parameterName || "",
    ...
    const handleCancelPolicy = async () => {
        ...
        const cancelMessage = keepRegistered
            ...
            : POLICY_MESSAGES.BASE_POLICY.WARNING
                  .CANCEL_WITH_COMPENSATION_CONFIRM;

        Modal.confirm({
            title: "Xác nhận huỷ gói bảo hiểm",
                ...
                try {
                    ...
                    message.success({
                        content: keepRegistered
                            ? POLICY_MESSAGES.BASE_POLICY.SUCCESS
                    ...
                    message.error({
                        content:
                            cancelError ||
                            POLICY_MESSAGES.BASE_POLICY.ERROR.CANCEL_FAILED,
                        key: "cancel-loading",
                    });
                }
            },
    ...
    const getStatusTag = (status) => {
        const statusConfig = {
            ...
            payment_due: {
                background: "#f8d9ff",
                color: "purple",
                border: "1px solid #BB8ED0",
                icon: <ClockCircleOutlined />,
                text: dict.getEnumLabel("BasePolicyStatus", "payment_due"),
            },
            active: {
    ...
    const tabItems = [
        ...
        },
        {
            ...
            children: (
                <TagsDetail policyData={policyDetail} mockData={mockData} />
            ),
        },
    ];
                    ...
                    <div>
                        ...
                        >
                            <Radio value="keep">
                                <div>
                                    <Text strong>
                                        Giữ nguyên các hợp đồng đã ký
                                    </Text>
                                    <br />
                                    <Text type="secondary">
                                        Huỷ chính sách nhưng các hợp đồng đã ký
                                        cho nông dân sẽ vẫn hoạt động bình
                                        thường. Không có bồi thường.
                                    </Text>
                                </div>
                            </Radio>
                            <Radio value="compensate">
                                <div>
                                    <Text strong>
                                        Huỷ các hợp đồng đã ký (có bồi thường)
                                    </Text>
                                    <br />
                                    <Text type="secondary">
                                        Huỷ chính sách và huỷ tất cả các hợp
                                        đồng đã ký. Nông dân sẽ được bồi thường
                                        và nhận được thông báo từ hệ thống.
                                    </Text>
                                </div>
                            </Radio>
                        </Radio.Group>
