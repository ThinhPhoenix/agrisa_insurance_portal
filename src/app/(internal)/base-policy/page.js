...
import { useCreatePayment } from "@/services/hooks/payment/use-create-payment";
import { useDataBillCost } from "@/services/hooks/policy/use-data-bill-cost";
import {
    CheckCircleOutlined,
    ClockCircleOutlined,
    DollarOutlined,
...
export default function PolicyPage() {
    ...
    // Use create payment hook
    const { createPayment, loading: paymentLoading } = useCreatePayment();

    // Use data bill cost hook
    const { fetchDataBillCost } = useDataBillCost();
    ...
    const {
        ...
        filterHandlers: {
            ...
            premiumRange: (item, value) => {
                ...
                return (
                    item.premiumBaseRate >= min && item.premiumBaseRate < max
                );
            },
            durationRange: (item, value) => {
                ...
                const [min, max] = value.split("-").map(Number);
                return (
                    item.coverageDurationDays >= min &&
                    item.coverageDurationDays < max
                );
            },
    ...
    const filterOptions = {
        ...
        policyStatuses: [
            ...
            {
                label: dict.getEnumLabel("BasePolicyStatus", "draft"),
                value: "draft",
            },
            {
                label: dict.getEnumLabel("BasePolicyStatus", "payment_due"),
                value: "payment_due",
            },
            {
    ...
    const handleCreatePayment = async (policy) => {
        try {
            let dataCost = await fetchDataBillCost(policy.id);
            const paymentData = await createPayment({
                amount: dataCost.data,
                // amount: 2000,
                description: `TT ${policy.productCode}`,
                type: "data_bill_payment",
                items: [
                    {
                        item_id: policy.id,
