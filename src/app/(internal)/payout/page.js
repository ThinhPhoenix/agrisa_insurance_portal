...
export default function PayoutListPage() {
        ...
        selections: [
            {
                ...
                onSelect: () => {
                    setSelectedRowKeys(payouts?.map((item) => item.id) || []);
                    setSelectedRows(payouts || []);
                },
            },
            {
    ...
    const handleBulkVerify = async () => {
        ...
        // Extract registered_policy_id from selected rows
        const registeredPolicyIds = selectedRows
            .map((row) => row.registered_policy_id)
        ...
        if (result) {
            ...
            message.error(`Xác minh thất bại: ${bulkVerifyError}`);
            console.error("Bulk verification failed:", bulkVerifyError);
        }
    };
                    ...
                    <div>
                        <div className="flex justify-start items-center gap-2 mb-2">
                            ...
                            {selectedRowKeys.length > 0 && (
                                ...
                                >
                                    Chi trả đã chọn ({selectedRowKeys.length})
                                </Button>
                            )}
                        </div>
