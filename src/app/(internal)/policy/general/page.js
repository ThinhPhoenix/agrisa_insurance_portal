...
import { useAllPolicies } from "@/services/hooks/policy/use-all-policies";
...
export default function GeneralPoliciesPage() {
        ...
        onOk: async () => {
          ...
          try {
            ...
            for (let i = 0; i < imageFieldCount; i++) {
              ...
              if (mode === "file" && selectedFiles[i]) {
                ...
                if (!uploadResult.success) {
                  ...
                  message.error(
                    uploadResult.error ||
                      CANCEL_REQUEST_MESSAGES.ERROR.IMAGE_UPLOAD_FAILED
                  );
            ...
            if (result.success) {
              ...
              message.error(
                result.error || CANCEL_REQUEST_MESSAGES.ERROR.CREATE_FAILED
              );
            }
          } catch (error) {
            ...
            message.destroy("creating");
            message.error(
              error.message || CANCEL_REQUEST_MESSAGES.ERROR.UNKNOWN_ERROR
            );
          } finally {
            setSubmittingCancel(false);
          }
    ...
    },
    {
      ...
      render: (_, record) => (
        <div className="insurance-actions-cell">
          <Space size="small">
            <Link href={`/policy/policy-detail?id=${record.id}&type=general`}>
              <Button
                type="dashed"
                size="small"
  ...
  // Search fields - no preset status filter (shows all policies)
  const searchFields = [
    {
      name: "search",
      label: "Tìm kiếm",
    ...
    },
    {
      ...
      options: [
        ...
        { label: "Đang chi trả", value: "payout" },
        { label: "Hết hạn", value: "expired" },
        { label: "Chờ hủy", value: "pending_cancel" },
        { label: "Đã hủy", value: "cancelled" },
        { label: "Đã từ chối", value: "rejected" },
        { label: "Tranh chấp", value: "dispute" },
        {
          label: "Đã hủy - chờ thanh toán",
        ...
        <div className="insurance-header">
          <div>
            ...
            <Text className="insurance-subtitle">
              Danh sách và quản lý toàn bộ hợp đồng bảo hiểm
            </Text>
          </div>
        </div>
                    ...
                    children: (
                      <div className="space-y-4">
                          ...
                          {imageFieldCount > 1 && (
                            <Button
                              ...
                              onClick={() => {
                                ...
                                setImageUploadMode(
                                  imageUploadMode.slice(0, newCount)
                                );
                                ...
                                if (filePreviewUrls[imageFieldCount - 1]) {
                                  URL.revokeObjectURL(
                                    filePreviewUrls[imageFieldCount - 1]
                                  );
                                }
                                setFilePreviewUrls(
                                  filePreviewUrls.slice(0, newCount)
                                );
                              }}
                            >
                              - Xóa hình ảnh cuối
                        ...
                        <p className="text-sm text-gray-500">
                          Bạn có thể tải file hình ảnh lên hoặc nhập URL trực
                          tiếp. Có thể thêm nhiều hình ảnh minh họa cho yêu cầu.
                        </p>
                      </div>
                    ),
                  },
