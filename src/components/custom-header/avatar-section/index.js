...
export default function AvatarSection({ isMobile }) {
  ...
  // Use global message API from antd (consistent with rest of the codebase)
            ...
            label: (
              <div
                ...
                style={{
                  ...
                  cursor: "pointer",
                  backgroundColor:
                    readNotifications.has(item.id) || item.status === "read"
                      ? "transparent"
                      : getTypeColor(item.platform || "default"),
                  borderLeft:
                    readNotifications.has(item.id) || item.status === "read"
                      ? "none"
    ...
    <div className="flex items-center gap-2">
      ...
      >
        <Avatar
          ...
          alt={partnerData?.partner_display_name || user?.user?.full_name}
          style={{ cursor: "pointer" }}
        >
          {partnerData?.partner_display_name
            ? partnerData.partner_display_name.charAt(0).toUpperCase()
            : user?.user?.full_name
