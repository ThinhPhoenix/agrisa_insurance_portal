 ...
 * Mapping với backend server responses (base_policy_json_spec_vi.md)
 */
...
export const POLICY_MESSAGES = {
  ...
  BASE_POLICY: {
    ...
    ERROR: {
      ...
      PREMIUM_BASE_RATE_REQUIRED: "Vui lòng nhập tỷ lệ phí cơ bản khi không có phí cố định!",
      ...
      OVER_THRESHOLD_MULTIPLIER_NEGATIVE: "Hệ số nhân vượt ngưỡng không được âm!",
      PAYOUT_CAP_NEGATIVE: "Giới hạn chi trả tối đa không được âm!",

      // Status validation
      INVALID_STATUS: "Trạng thái không hợp lệ! (Phải là: draft, active, hoặc archived)",
      INVALID_DOCUMENT_VALIDATION_STATUS: "Trạng thái xác thực tài liệu không hợp lệ! (Phải là: pending, passed, passed_ai, failed, hoặc warning)",

      // Dates validation
      ENROLLMENT_START_AFTER_END: "Ngày bắt đầu đăng ký phải trước ngày kết thúc!",
      INSURANCE_VALID_FROM_AFTER_TO: "Ngày bắt đầu hiệu lực phải trước ngày kết thúc!",
      ENROLLMENT_END_AFTER_VALID_FROM: "Ngày kết thúc đăng ký phải trước hoặc bằng ngày bắt đầu hiệu lực!",

      // Cancel rate validation
      CANCEL_PREMIUM_RATE_INVALID: "Tỷ lệ phí hủy phải từ 0 đến 1 (0% - 100%)!",
      RENEWAL_DISCOUNT_RATE_INVALID: "Tỷ lệ giảm giá gia hạn phải từ 0 đến 1 (0% - 100%)!",

      // Document validation
      POLICY_DOCUMENT_DATA_INVALID: "Dữ liệu tài liệu không hợp lệ! Phải là chuỗi base64.",
      ...
      PRODUCT_CODE_INVALID_FORMAT: "Mã sản phẩm chỉ chứa chữ hoa, số và dấu gạch dưới!",
      CURRENCY_INVALID: "Mã tiền tệ không hợp lệ! (VD: VND, USD, EUR)",
    },

    VALIDATION: {
      PRODUCT_NAME_MIN_LENGTH: "Tên sản phẩm phải có ít nhất 3 ký tự!",
      PRODUCT_CODE_FORMAT: "Mã sản phẩm chỉ được chứa chữ hoa, số và dấu gạch dưới (_)!",
      ...
      IS_PAYOUT_PER_HECTARE_REQUIRED: "Vui lòng chọn cách tính chi trả theo diện tích!",
      INSURANCE_VALID_FROM_REQUIRED: "Vui lòng chọn ngày bắt đầu hiệu lực (bắt buộc)!",
      INSURANCE_VALID_TO_REQUIRED: "Vui lòng chọn ngày kết thúc hiệu lực (bắt buộc)!",
    },
    ...
    WARNING: {
      ...
      MISSING_PREMIUM_RATE: "Cần nhập tỷ lệ phí cơ bản khi không có phí cố định!",
      PAYOUT_CAP_LOWER_THAN_FIX: "Giới hạn chi trả thấp hơn số tiền chi trả cố định!",
    },
  },
  ...
  TRIGGER: {
    ...
    ERROR: {
      ...
      INVALID_LOGICAL_OPERATOR: "Toán tử logic không hợp lệ! (Phải là AND hoặc OR)",
      MONITOR_INTERVAL_INVALID: "Khoảng thời gian giám sát phải lớn hơn 0!",
      INVALID_MONITOR_FREQUENCY_UNIT: "Đơn vị thời gian không hợp lệ! (Phải là: hour, day, week, month, year)",
      BLACKOUT_PERIODS_INVALID: "Khoảng thời gian không giám sát không hợp lệ!",
      BLACKOUT_PERIODS_OVERLAP: "Các khoảng thời gian không giám sát không được trùng lặp!",
    },
  ...
  CONDITION: {
    ...
    ERROR: {
      ...
      INVALID_THRESHOLD_OPERATOR: "Toán tử ngưỡng không hợp lệ! (Phải là: <, >, <=, >=, ==, !=, change_gt, change_lt)",
      INVALID_AGGREGATION_FUNCTION: "Hàm tổng hợp không hợp lệ! (Phải là: sum, avg, min, max, change)",
      ...
      DATA_SOURCE_NOT_EXIST: "Nguồn dữ liệu không tồn tại hoặc chưa được kích hoạt!",
      ...
      DATA_SOURCE_ALREADY_USED: "Nguồn dữ liệu này đã được sử dụng trong điều kiện khác!",
      BASELINE_FUNCTION_REQUIRED: "Vui lòng chọn hàm baseline khi đã nhập cửa sổ baseline!",
      BASELINE_WINDOW_TOO_SMALL: "Cửa sổ baseline nên lớn hơn hoặc bằng cửa sổ tổng hợp!",
    },
    ...
    WARNING: {
      ...
      BASELINE_WITHOUT_WINDOW: "Đã chọn hàm baseline nhưng chưa nhập cửa sổ baseline!",
    },
  },
